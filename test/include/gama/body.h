...
typedef struct {
  uint8_t is_active; /**< Whether the body is active in the physics simulation.
                        Set to 0 to disable. */
  uint8_t is_static; /**< Whether the body is static (immovable). Static bodies
                        do not move or respond to forces. */
  ...
  double width, height,
      radius;  /**< Dimensions of the body (width/height for
                  rectangles, radius for circles). For circles,
                  width and height are typically set to 2*radius. */
  double mass; /**< Mass of the body. Set to 0 for infinite mass (similar to
                  static but can move). */
  double restitution; /**< Bounciness of the body (0.0 = no bounce, 1.0+ =
                         more bounce). */
  double
      friction; /**< Friction coefficient (not currently used in the code). */
} gmBody;

/**
 * @brief Creates a new physics body with specified properties.
 *
 * The `radius` for circular colliders is automatically set based on `w` (or `h`
 * if smaller). For rectangular colliders, `radius` is set to `w < h ? w : h`.
 *
 ...
 * @param c The type of collider for the body (`GM_COLLIDER_RECT` or
 * `GM_COLLIDER_CIRCLE`).
 * @return A new `gmBody` instance initialized with default active/non-static
 * properties.
 */
...
void gm_max_speed(gmBody *body, double max_speed) {
  double current_speed = gm_pos_magnitude(body->velocity);
 ...
 * its velocity is scaled up to match `min_speed` while preserving its
 * direction. If the body is stationary (speed is 0), its velocity remains 0.
 *
 * @param body Pointer to the body to modify.
...
/**
 * @brief Sets the speed of a body using an animation function, preserving its
 * direction.
 *
 * This function applies a smooth animation to the body's velocity components
 ...
 */
gmBody gm_rect_body(double m, double x, double y, double w, double h) {
  return gm_body_create(m, x, y, w, h, GM_COLLIDER_RECT);
}
// backward compatibility
#define gm_rectangle_body gm_rect_body
...
gmBody gm_circle_body(double m, double x, double y, double r) {
  return gm_body_create(m, x, y, r * 2, r * 2,
                        GM_COLLIDER_CIRCLE); // Pass diameter for w/h
}
 ...
 */
static inline int gm_body_hovered(gmBody *body) {
 ...
 */
static inline int gm_body_clicked(gmBody *body) {
  return gm_mouse.clicked && gm_body_hovered(body);
}

/**
 * @brief Constrains a body within specified rectangular boundaries by clipping
 * its position.
 *
 ...
 * @param bx The beginning x-coordinate of the boundary. If 0, the left boundary
 * is ignored.
 * @param ex The ending x-coordinate of the boundary. If 0, the right boundary
 * is ignored.
 * @param by The beginning y-coordinate of the boundary. If 0, the bottom
 * boundary is ignored.
 * @param ey The ending y-coordinate of the boundary. If 0, the top boundary is
 * ignored.
 ...
 * @param by The beginning y-coordinate of the boundary.
 * @param ey The ending y-coordinate of the boundary.
 * @param restitution The bounciness factor to apply (0.0 for no bounce, 1.0 for
 * perfect bounce).
 * @return A bitmask indicating which boundaries were exceeded:
 *         - Bit 3 (0b1000): Left boundary (`body->position.x < bx`)
 *         - Bit 2 (0b0100): Right boundary (`body->position.x > ex`)
