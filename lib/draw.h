...
int32_t gama_draw_line(double x1, double y1, double x2, double y2,
                       double thickness, gama_color c) {
  return gapi_draw_line(x1, y1, x2, y2, thickness, gama_color_red(c),
                        gama_color_green(c), gama_color_blue(c),
                        gama_color_alpha(c));
}

int32_t gama_draw_rectangle(double x, double y, double w, double h,
                            gama_color c) {
...
int32_t gama_draw_circle(double center_x, double center_y, double radius,
                         gama_color c) {
...
int32_t gama_draw_ellipse(double x, double y, double w, double h,
                          gama_color c) {
  return gapi_draw_ellipse(x, y, w, h, gama_color_red(c), gama_color_green(c),
                           gama_color_blue(c), gama_color_alpha(c));
}

int32_t gama_draw_triangle(double x1, double y1, double x2, double y2,
                           double x3, double y3, gama_color c) {
  return gapi_draw_triangle(x1, y1, x2, y2, x3, y3, gama_color_red(c),
                            gama_color_green(c), gama_color_blue(c),
                            gama_color_alpha(c));
}

int32_t gama_draw_image(gama_image img, double x, double y, double w,
                        double h) {
...
int32_t gama_draw_text(double x, double y, const char *text, double font_size,
                       gama_color c) {
...
void gama_draw_body(const gama_body *body, gama_color c) {
  ...
  case GAMA_COLLIDER_RECT:
    gama_draw_rectangle(body->position.x, body->position.y, body->width,
                        body->height, c);
    break;
...
void gama_draw_round_rect_body(const gama_body *body, double radius_factor,
                               gama_color c) {
  if (body == NULL || !body->is_active)
    return;
  double radius =
      (body->width < body->height ? body->width : body->height) * radius_factor;
  gama_draw_rounded_rectangle(body->position.x, body->position.y, body->width,
...
void gama_draw_circle_body(const gama_body *body, gama_color c) {
  if (body == NULL || !body->is_active)
    return;
  double radius = body->collider_type == GAMA_COLLIDER_CIRCLE
                      ? body->radius
...
void gama_draw_ellipse_body(const gama_body *body, gama_color c) {
  if (body == NULL || !body->is_active)
    return;
  gama_draw_ellipse(body->position.x, body->position.y, body->width,
                    body->height, c);
}

void gama_draw_triangle_body(const gama_body *body, double x2_offset,
                             double y2_offset, double x3_offset,
                             double y3_offset, gama_color c) {
  if (body == NULL || !body->is_active)
    return;
...
void gama_draw_image_body(const gama_body *body, gama_image img) {
  if (body == NULL || !body->is_active)
    return;
  gama_draw_image(img, body->position.x, body->position.y, body->width,
                  body->height);
}

void gama_draw_text_body(const gama_body *body, const char *text,
                         double font_size, gama_color c) {
  if (body == NULL || !body->is_active)
    return;
...
void gama_render_body(gama_body *body, gama_color c) {
  if (body == NULL) {
    return;
  }
  gama_body_update(body);  // Physics update
  gama_draw_body(body, c); // Generic draw
}

// Convenience function to update physics and then draw a body as a rectangle
void gama_render_rect_body(gama_body *body, gama_color c) {
  if (body == NULL)
    return;
  gama_body_update(body);
  gama_draw_rect_body(body, c);
}

// Convenience function to update physics and then draw a body as a rounded
// rectangle
void gama_render_round_rect_body(gama_body *body, double radius_factor,
                                 gama_color c) {
  if (body == NULL)
    return;
  gama_body_update(body);
...
void gama_render_circle_body(gama_body *body, gama_color c) {
  if (body == NULL)
    return;
  gama_body_update(body);
...
void gama_render_ellipse_body(gama_body *body, gama_color c) {
  if (body == NULL)
    return;
  gama_body_update(body);
...
void gama_render_triangle_body(gama_body *body, double x2_offset,
                               double y2_offset, double x3_offset,
                               double y3_offset, gama_color c) {
  if (body == NULL)
    return;
...
void gama_render_image_body(gama_body *body, gama_image img) {
  if (body == NULL)
    return;
  gama_body_update(body);
...
void gama_render_text_body(gama_body *body, const char *text, double font_size,
                           gama_color c) {
  if (body == NULL)
    return;
  gama_body_update(body);
