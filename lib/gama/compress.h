...
int gm_compress_to(const char *data, int data_len, char *dest,
                   size_t dest_len) {
...
gmCompressed *gm_compress(const char *data, size_t data_len) {
...
int gm_decompress_to(const char *compressed, size_t compressed_len, char *dest,
                     char dest_len) {
...
char *gm_decompress(const gmCompressed *compressed) {
  char *dest = malloc(compressed->original);
  if (!dest)
    return NULL;
